/* Includes ----------------------------------------------------------------- */
#include "bsp/can.h"

#include <cmsis_os2.h>
#include <string.h>

/* Private define ----------------------------------------------------------- */
#define CAN_QUEUE_MUTEX_TIMEOUT 100

/* Private typedef ---------------------------------------------------------- */
typedef struct BSP_CAN_QueueNode {
  BSP_CAN_t can;
  uint32_t can_id;
  osMessageQueueId_t queue;
  struct BSP_CAN_QueueNode *next;
} BSP_CAN_QueueNode_t;

/* Private variables -------------------------------------------------------- */
static BSP_CAN_QueueNode_t *queue_list = NULL;
static osMutexId_t queue_mutex = NULL;
static void (*can_callback[BSP_CAN_NUM][BSP_CAN_CB_NUM])(void);
static BSP_CAN_IdParser_t id_parser = NULL;
static BSP_CAN_TxQueue_t tx_queues[BSP_CAN_NUM];
static bool inited = false;

/* Private function prototypes ---------------------------------------------- */
static BSP_CAN_t CAN_Get(CAN_HandleTypeDef *hcan);
static osMessageQueueId_t BSP_CAN_FindQueue(BSP_CAN_t can, uint32_t can_id);
static int8_t BSP_CAN_CreateIdQueue(BSP_CAN_t can, uint32_t can_id, uint8_t queue_size);
static void BSP_CAN_RxFifo0Callback(void);
static void BSP_CAN_TxCompleteCallback(void);
static BSP_CAN_FrameType_t BSP_CAN_GetFrameType(CAN_RxHeaderTypeDef *header);
static uint32_t BSP_CAN_DefaultIdParser(uint32_t original_id, BSP_CAN_FrameType_t frame_type);
static void BSP_CAN_TxQueueInit(BSP_CAN_t can);
static bool BSP_CAN_TxQueuePush(BSP_CAN_t can, BSP_CAN_TxMessage_t *msg);
static bool BSP_CAN_TxQueuePop(BSP_CAN_t can, BSP_CAN_TxMessage_t *msg);
static bool BSP_CAN_TxQueueIsEmpty(BSP_CAN_t can);

/* Private functions -------------------------------------------------------- */
static BSP_CAN_t CAN_Get(CAN_HandleTypeDef *hcan) {
  if (hcan == NULL) {
    return BSP_CAN_ERR;
  }
  if (hcan->Instance == CAN2) {
    return BSP_CAN_2;
  }
  return BSP_CAN_ERR;
}

static osMessageQueueId_t BSP_CAN_FindQueue(BSP_CAN_t can, uint32_t can_id) {
  BSP_CAN_QueueNode_t *node = queue_list;
  while (node != NULL) {
    if (node->can == can && node->can_id == can_id) {
      return node->queue;
    }
    node = node->next;
  }
  return NULL;
}

static int8_t BSP_CAN_CreateIdQueue(BSP_CAN_t can, uint32_t can_id, uint8_t queue_size) {
  BSP_CAN_QueueNode_t *new_node = NULL;

  if (queue_size == 0) {
    queue_size = BSP_CAN_DEFAULT_QUEUE_SIZE;
  }

  if (osMutexAcquire(queue_mutex, CAN_QUEUE_MUTEX_TIMEOUT) != osOK) {
    return BSP_ERR_TIMEOUT;
  }

  if (BSP_CAN_FindQueue(can, can_id) != NULL) {
    osMutexRelease(queue_mutex);
    return BSP_ERR;
  }

  new_node = (BSP_CAN_QueueNode_t *)BSP_Malloc(sizeof(BSP_CAN_QueueNode_t));
  if (new_node == NULL) {
    osMutexRelease(queue_mutex);
    return BSP_ERR_NULL;
  }

  new_node->queue = osMessageQueueNew(queue_size, sizeof(BSP_CAN_Message_t), NULL);
  if (new_node->queue == NULL) {
    BSP_Free(new_node);
    osMutexRelease(queue_mutex);
    return BSP_ERR;
  }

  new_node->can = can;
  new_node->can_id = can_id;
  new_node->next = queue_list;
  queue_list = new_node;

  osMutexRelease(queue_mutex);
  return BSP_OK;
}

static BSP_CAN_FrameType_t BSP_CAN_GetFrameType(CAN_RxHeaderTypeDef *header) {
  if (header->RTR == CAN_RTR_REMOTE) {
    return (header->IDE == CAN_ID_EXT) ? BSP_CAN_FRAME_EXT_REMOTE : BSP_CAN_FRAME_STD_REMOTE;
  }
  return (header->IDE == CAN_ID_EXT) ? BSP_CAN_FRAME_EXT_DATA : BSP_CAN_FRAME_STD_DATA;
}

static uint32_t BSP_CAN_DefaultIdParser(uint32_t original_id, BSP_CAN_FrameType_t frame_type) {
  (void)frame_type;
  return original_id;
}

static void BSP_CAN_TxQueueInit(BSP_CAN_t can) {
  if (can >= BSP_CAN_NUM) {
    return;
  }
  tx_queues[can].head = 0;
  tx_queues[can].tail = 0;
}

static bool BSP_CAN_TxQueuePush(BSP_CAN_t can, BSP_CAN_TxMessage_t *msg) {
  BSP_CAN_TxQueue_t *queue = NULL;
  uint32_t next_head = 0;

  if (can >= BSP_CAN_NUM || msg == NULL) {
    return false;
  }

  queue = &tx_queues[can];
  next_head = (queue->head + 1u) % BSP_CAN_TX_QUEUE_SIZE;
  if (next_head == queue->tail) {
    return false;
  }

  queue->buffer[queue->head] = *msg;
  queue->head = next_head;
  return true;
}

static bool BSP_CAN_TxQueuePop(BSP_CAN_t can, BSP_CAN_TxMessage_t *msg) {
  BSP_CAN_TxQueue_t *queue = NULL;

  if (can >= BSP_CAN_NUM || msg == NULL) {
    return false;
  }

  queue = &tx_queues[can];
  if (queue->head == queue->tail) {
    return false;
  }

  *msg = queue->buffer[queue->tail];
  queue->tail = (queue->tail + 1u) % BSP_CAN_TX_QUEUE_SIZE;
  return true;
}

static bool BSP_CAN_TxQueueIsEmpty(BSP_CAN_t can) {
  if (can >= BSP_CAN_NUM) {
    return true;
  }
  return tx_queues[can].head == tx_queues[can].tail;
}

static void BSP_CAN_TxCompleteCallback(void) {
  BSP_CAN_TxMessage_t msg;
  uint32_t mailbox = 0;

  while (!BSP_CAN_TxQueueIsEmpty(BSP_CAN_2)) {
    if (HAL_CAN_GetTxMailboxesFreeLevel(&hcan2) == 0u) {
      break;
    }
    if (!BSP_CAN_TxQueuePop(BSP_CAN_2, &msg)) {
      break;
    }
    if (HAL_CAN_AddTxMessage(&hcan2, &msg.header, msg.data, &mailbox) != HAL_OK) {
      break;
    }
  }
}

static void BSP_CAN_RxFifo0Callback(void) {
  CAN_RxHeaderTypeDef rx_header;
  uint8_t rx_data[BSP_CAN_MAX_DLC];

  while (HAL_CAN_GetRxFifoFillLevel(&hcan2, CAN_RX_FIFO0) > 0u) {
    osMessageQueueId_t queue = NULL;
    BSP_CAN_Message_t msg;
    uint32_t original_id = 0;

    if (HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, &rx_header, rx_data) != HAL_OK) {
      break;
    }

    original_id = (rx_header.IDE == CAN_ID_STD) ? rx_header.StdId : rx_header.ExtId;
    queue = BSP_CAN_FindQueue(BSP_CAN_2, BSP_CAN_ParseId(original_id, BSP_CAN_GetFrameType(&rx_header)));
    if (queue == NULL) {
      continue;
    }

    memset(&msg, 0, sizeof(msg));
    msg.frame_type = BSP_CAN_GetFrameType(&rx_header);
    msg.original_id = original_id;
    msg.parsed_id = BSP_CAN_ParseId(original_id, msg.frame_type);
    msg.dlc = rx_header.DLC;
    if (rx_header.RTR == CAN_RTR_DATA && rx_header.DLC > 0u) {
      memcpy(msg.data, rx_data, rx_header.DLC);
    }
    msg.timestamp = HAL_GetTick();
    osMessageQueuePut(queue, &msg, 0u, BSP_CAN_TIMEOUT_IMMEDIATE);
  }
}

/* HAL callback bridge ------------------------------------------------------ */
void HAL_CAN_TxMailbox0CompleteCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_TX_MAILBOX0_CPLT_CB] != NULL) {
    can_callback[can][HAL_CAN_TX_MAILBOX0_CPLT_CB]();
  }
}

void HAL_CAN_TxMailbox1CompleteCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_TX_MAILBOX1_CPLT_CB] != NULL) {
    can_callback[can][HAL_CAN_TX_MAILBOX1_CPLT_CB]();
  }
}

void HAL_CAN_TxMailbox2CompleteCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_TX_MAILBOX2_CPLT_CB] != NULL) {
    can_callback[can][HAL_CAN_TX_MAILBOX2_CPLT_CB]();
  }
}

void HAL_CAN_TxMailbox0AbortCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_TX_MAILBOX0_ABORT_CB] != NULL) {
    can_callback[can][HAL_CAN_TX_MAILBOX0_ABORT_CB]();
  }
}

void HAL_CAN_TxMailbox1AbortCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_TX_MAILBOX1_ABORT_CB] != NULL) {
    can_callback[can][HAL_CAN_TX_MAILBOX1_ABORT_CB]();
  }
}

void HAL_CAN_TxMailbox2AbortCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_TX_MAILBOX2_ABORT_CB] != NULL) {
    can_callback[can][HAL_CAN_TX_MAILBOX2_ABORT_CB]();
  }
}

void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_RX_FIFO0_MSG_PENDING_CB] != NULL) {
    can_callback[can][HAL_CAN_RX_FIFO0_MSG_PENDING_CB]();
  }
}

void HAL_CAN_RxFifo0FullCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_RX_FIFO0_FULL_CB] != NULL) {
    can_callback[can][HAL_CAN_RX_FIFO0_FULL_CB]();
  }
}

void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_RX_FIFO1_MSG_PENDING_CB] != NULL) {
    can_callback[can][HAL_CAN_RX_FIFO1_MSG_PENDING_CB]();
  }
}

void HAL_CAN_RxFifo1FullCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_RX_FIFO1_FULL_CB] != NULL) {
    can_callback[can][HAL_CAN_RX_FIFO1_FULL_CB]();
  }
}

void HAL_CAN_SleepCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_SLEEP_CB] != NULL) {
    can_callback[can][HAL_CAN_SLEEP_CB]();
  }
}

void HAL_CAN_WakeUpFromRxMsgCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_WAKEUP_FROM_RX_MSG_CB] != NULL) {
    can_callback[can][HAL_CAN_WAKEUP_FROM_RX_MSG_CB]();
  }
}

void HAL_CAN_ErrorCallback(CAN_HandleTypeDef *hcan) {
  BSP_CAN_t can = CAN_Get(hcan);
  if (can != BSP_CAN_ERR && can_callback[can][HAL_CAN_ERROR_CB] != NULL) {
    can_callback[can][HAL_CAN_ERROR_CB]();
  }
}

/* Exported functions ------------------------------------------------------- */
int8_t BSP_CAN_Init(void) {
  CAN_FilterTypeDef filter = {0};
  int i = 0;

  if (inited) {
    return BSP_ERR_INITED;
  }

  memset(can_callback, 0, sizeof(can_callback));
  for (i = 0; i < BSP_CAN_NUM; ++i) {
    BSP_CAN_TxQueueInit((BSP_CAN_t)i);
  }

  queue_mutex = osMutexNew(NULL);
  if (queue_mutex == NULL) {
    return BSP_ERR;
  }

  id_parser = BSP_CAN_DefaultIdParser;
  inited = true;

  filter.FilterBank = 0;
  filter.FilterMode = CAN_FILTERMODE_IDMASK;
  filter.FilterScale = CAN_FILTERSCALE_32BIT;
  filter.FilterIdHigh = 0;
  filter.FilterIdLow = 0;
  filter.FilterMaskIdHigh = 0;
  filter.FilterMaskIdLow = 0;
  filter.FilterFIFOAssignment = CAN_RX_FIFO0;
  filter.FilterActivation = ENABLE;
  filter.SlaveStartFilterBank = 14;

  if (HAL_CAN_ConfigFilter(&hcan2, &filter) != HAL_OK) {
    return BSP_ERR;
  }
  if (HAL_CAN_Start(&hcan2) != HAL_OK) {
    return BSP_ERR;
  }

  BSP_CAN_RegisterCallback(BSP_CAN_2, HAL_CAN_RX_FIFO0_MSG_PENDING_CB, BSP_CAN_RxFifo0Callback);
  BSP_CAN_RegisterCallback(BSP_CAN_2, HAL_CAN_TX_MAILBOX0_CPLT_CB, BSP_CAN_TxCompleteCallback);
  BSP_CAN_RegisterCallback(BSP_CAN_2, HAL_CAN_TX_MAILBOX1_CPLT_CB, BSP_CAN_TxCompleteCallback);
  BSP_CAN_RegisterCallback(BSP_CAN_2, HAL_CAN_TX_MAILBOX2_CPLT_CB, BSP_CAN_TxCompleteCallback);

  if (HAL_CAN_ActivateNotification(&hcan2, CAN_IT_RX_FIFO0_MSG_PENDING | CAN_IT_TX_MAILBOX_EMPTY) != HAL_OK) {
    return BSP_ERR;
  }

  return BSP_OK;
}

CAN_HandleTypeDef *BSP_CAN_GetHandle(BSP_CAN_t can) {
  switch (can) {
    case BSP_CAN_2:
      return &hcan2;
    default:
      return NULL;
  }
}

int8_t BSP_CAN_RegisterCallback(BSP_CAN_t can, BSP_CAN_Callback_t type, void (*callback)(void)) {
  if (!inited) {
    return BSP_ERR_INITED;
  }
  if (callback == NULL) {
    return BSP_ERR_NULL;
  }
  if (can >= BSP_CAN_NUM || type >= BSP_CAN_CB_NUM) {
    return BSP_ERR;
  }

  can_callback[can][type] = callback;
  return BSP_OK;
}

int8_t BSP_CAN_Transmit(BSP_CAN_t can, BSP_CAN_Format_t format, uint32_t id, uint8_t *data, uint8_t dlc) {
  CAN_HandleTypeDef *hcan = NULL;
  BSP_CAN_TxMessage_t tx_msg;
  uint32_t mailbox = 0;

  if (!inited) {
    return BSP_ERR_INITED;
  }
  if (can >= BSP_CAN_NUM || dlc > BSP_CAN_MAX_DLC) {
    return BSP_ERR;
  }
  if (data == NULL && format != BSP_CAN_FORMAT_STD_REMOTE && format != BSP_CAN_FORMAT_EXT_REMOTE) {
    return BSP_ERR_NULL;
  }

  hcan = BSP_CAN_GetHandle(can);
  if (hcan == NULL) {
    return BSP_ERR_NULL;
  }

  memset(&tx_msg, 0, sizeof(tx_msg));
  switch (format) {
    case BSP_CAN_FORMAT_STD_DATA:
      tx_msg.header.StdId = id;
      tx_msg.header.IDE = CAN_ID_STD;
      tx_msg.header.RTR = CAN_RTR_DATA;
      break;
    case BSP_CAN_FORMAT_EXT_DATA:
      tx_msg.header.ExtId = id;
      tx_msg.header.IDE = CAN_ID_EXT;
      tx_msg.header.RTR = CAN_RTR_DATA;
      break;
    case BSP_CAN_FORMAT_STD_REMOTE:
      tx_msg.header.StdId = id;
      tx_msg.header.IDE = CAN_ID_STD;
      tx_msg.header.RTR = CAN_RTR_REMOTE;
      break;
    case BSP_CAN_FORMAT_EXT_REMOTE:
      tx_msg.header.ExtId = id;
      tx_msg.header.IDE = CAN_ID_EXT;
      tx_msg.header.RTR = CAN_RTR_REMOTE;
      break;
    default:
      return BSP_ERR;
  }

  tx_msg.header.DLC = dlc;
  tx_msg.header.TransmitGlobalTime = DISABLE;
  if (data != NULL && dlc > 0u) {
    memcpy(tx_msg.data, data, dlc);
  }

  if (HAL_CAN_GetTxMailboxesFreeLevel(hcan) > 0u &&
      HAL_CAN_AddTxMessage(hcan, &tx_msg.header, tx_msg.data, &mailbox) == HAL_OK) {
    return BSP_OK;
  }

  return BSP_CAN_TxQueuePush(can, &tx_msg) ? BSP_OK : BSP_ERR;
}

int8_t BSP_CAN_TransmitStdDataFrame(BSP_CAN_t can, BSP_CAN_StdDataFrame_t *frame) {
  if (frame == NULL) {
    return BSP_ERR_NULL;
  }
  return BSP_CAN_Transmit(can, BSP_CAN_FORMAT_STD_DATA, frame->id, frame->data, frame->dlc);
}

int8_t BSP_CAN_TransmitExtDataFrame(BSP_CAN_t can, BSP_CAN_ExtDataFrame_t *frame) {
  if (frame == NULL) {
    return BSP_ERR_NULL;
  }
  return BSP_CAN_Transmit(can, BSP_CAN_FORMAT_EXT_DATA, frame->id, frame->data, frame->dlc);
}

int8_t BSP_CAN_TransmitRemoteFrame(BSP_CAN_t can, BSP_CAN_RemoteFrame_t *frame) {
  BSP_CAN_Format_t format;

  if (frame == NULL) {
    return BSP_ERR_NULL;
  }
  format = frame->is_extended ? BSP_CAN_FORMAT_EXT_REMOTE : BSP_CAN_FORMAT_STD_REMOTE;
  return BSP_CAN_Transmit(can, format, frame->id, NULL, frame->dlc);
}

int32_t BSP_CAN_GetTxQueueCount(BSP_CAN_t can) {
  BSP_CAN_TxQueue_t *queue = NULL;

  if (can >= BSP_CAN_NUM) {
    return -1;
  }
  queue = &tx_queues[can];
  if (queue->head >= queue->tail) {
    return (int32_t)(queue->head - queue->tail);
  }
  return (int32_t)(BSP_CAN_TX_QUEUE_SIZE - queue->tail + queue->head);
}

int8_t BSP_CAN_FlushTxQueue(BSP_CAN_t can) {
  if (can >= BSP_CAN_NUM) {
    return BSP_ERR;
  }
  tx_queues[can].head = 0;
  tx_queues[can].tail = 0;
  return BSP_OK;
}

int8_t BSP_CAN_RegisterId(BSP_CAN_t can, uint32_t can_id, uint8_t queue_size) {
  if (!inited) {
    return BSP_ERR_INITED;
  }
  return BSP_CAN_CreateIdQueue(can, can_id, queue_size);
}

int8_t BSP_CAN_GetMessage(BSP_CAN_t can, uint32_t can_id, BSP_CAN_Message_t *msg, uint32_t timeout) {
  osMessageQueueId_t queue = NULL;

  if (!inited) {
    return BSP_ERR_INITED;
  }
  if (msg == NULL) {
    return BSP_ERR_NULL;
  }
  if (osMutexAcquire(queue_mutex, CAN_QUEUE_MUTEX_TIMEOUT) != osOK) {
    return BSP_ERR_TIMEOUT;
  }

  queue = BSP_CAN_FindQueue(can, can_id);
  osMutexRelease(queue_mutex);

  if (queue == NULL) {
    return BSP_ERR_NO_DEV;
  }
  return (osMessageQueueGet(queue, msg, NULL, timeout) == osOK) ? BSP_OK : BSP_ERR;
}

int32_t BSP_CAN_GetQueueCount(BSP_CAN_t can, uint32_t can_id) {
  osMessageQueueId_t queue = NULL;

  if (!inited) {
    return -1;
  }
  if (osMutexAcquire(queue_mutex, CAN_QUEUE_MUTEX_TIMEOUT) != osOK) {
    return -1;
  }

  queue = BSP_CAN_FindQueue(can, can_id);
  osMutexRelease(queue_mutex);

  if (queue == NULL) {
    return -1;
  }
  return (int32_t)osMessageQueueGetCount(queue);
}

int8_t BSP_CAN_FlushQueue(BSP_CAN_t can, uint32_t can_id) {
  osMessageQueueId_t queue = NULL;
  BSP_CAN_Message_t msg;

  if (!inited) {
    return BSP_ERR_INITED;
  }
  if (osMutexAcquire(queue_mutex, CAN_QUEUE_MUTEX_TIMEOUT) != osOK) {
    return BSP_ERR_TIMEOUT;
  }

  queue = BSP_CAN_FindQueue(can, can_id);
  osMutexRelease(queue_mutex);

  if (queue == NULL) {
    return BSP_ERR_NO_DEV;
  }

  while (osMessageQueueGet(queue, &msg, NULL, BSP_CAN_TIMEOUT_IMMEDIATE) == osOK) {
  }
  return BSP_OK;
}

int8_t BSP_CAN_RegisterIdParser(BSP_CAN_IdParser_t parser) {
  if (!inited) {
    return BSP_ERR_INITED;
  }
  if (parser == NULL) {
    return BSP_ERR_NULL;
  }
  id_parser = parser;
  return BSP_OK;
}

uint32_t BSP_CAN_ParseId(uint32_t original_id, BSP_CAN_FrameType_t frame_type) {
  if (id_parser != NULL) {
    return id_parser(original_id, frame_type);
  }
  return BSP_CAN_DefaultIdParser(original_id, frame_type);
}
